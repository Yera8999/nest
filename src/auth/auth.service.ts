...
export class AuthService {
  ...
  async signIn(dto: SignInDto, response: Response) {
    ...
    await response.cookie("access", accessToken, {httpOnly: true, maxAge: 2141421421412, sameSite: 'none', secure: true});
    await response.cookie("refresh", refreshToken, {httpOnly: true, maxAge: 2141421421412, sameSite: 'none', secure: true});
  ...
  async refreshToken(request: Request, response: Response) {
    ...
    await user.save();
    await response.cookie("access", newAccessToken, {httpOnly: true, maxAge: 10000000, sameSite: 'none', secure: true });
    await response.cookie("refresh", newRefreshToken, {httpOnly: true, maxAge: 10000000, sameSite: 'none', secure: true});
    return { message: "success" };
  }
